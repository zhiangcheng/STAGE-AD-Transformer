# Create tables of gene annotations for male GWAS

directory = "/path/07_FUMA/Downloaded_results/MDD_Metaanalysis_Freeze3_Males_genomewidesig_positionalmap10kb_MHCexcl/"

snps_male = "/path/07_FUMA/Downloaded_results/MDD_Metaanalysis_Freeze3_Males_genomewidesig_positionalmap10kb_MHCexcl/snps.txt"

genes_male = "/path/07_FUMA/Downloaded_results/MDD_Metaanalysis_Freeze3_Males_genomewidesig_positionalmap10kb_MHCexcl/genes.txt"
...
genes_male_df <- read.table(genes_male, header = T, stringsAsFactors = F)

males_GWAS_df <- read.table(femaleGWAS, header = T, stringsAsFactors = F)
...
# For some genes there are two SNPs in the IndSigSNPs column. Split these into their own row.
genes_male_df_split <- genes_male_df %>%
  separate_rows(IndSigSNPs, sep = ":")

genes_male_df_split <- genes_male_df_split %>%
  separate_rows(IndSigSNPs, sep = ";")

genes_male_eqtl <- genes_male_df_split %>%
...
genes_male_ci <- genes_male_df_split %>%
...
genes_male_pos <- genes_male_df_split %>%
...
genes_male_all <- snps_male_LD %>%
  full_join(genes_male_pos) %>%
  full_join(genes_male_eqtl) %>%
  full_join(genes_male_ci)
...
snps_genes_male <- genes_male_all %>%
  left_join(males_GWAS_df2, by = c("IndSigSNPs" = "rsID_build37"))

snps_genes_male <- snps_genes_male %>%
...
# Add female GWAS sumstats info
females_GWAS_df2 <- females_GWAS_df %>%
  mutate(Allele1 = toupper(Allele1),
         Allele2 = toupper(Allele2))

snps_genes_male <- snps_genes_male %>%
  left_join(females_GWAS_df2, by = c("rsID" = "rsID_build37", "Effect Allele" = "Allele1", "Other Allele" = "Allele2"))
...
# Save
outfile <- paste(directory, "Males_sig_SNPs_Malesinfo_Femalesinfo_GxSinfo_annotations.txt", sep="")
write.table(snps_genes_male, file = outfile, sep = "\t", row.names = FALSE, quote = FALSE)
...
genes_2plus_methods_male <- genes_male_df_split %>%
...
genes_nearest_snps_LD_male <- snps_male_df %>%
...
genes_nearest_lead_snps_male <- snps_male_df %>%
...
# Combine dataframes
genes_male_combined <- genes_nearest_lead_snps_male %>%
  full_join(genes_nearest_snps_LD_male) %>%
  full_join(genes_2plus_methods_male)

# Save
outfile <- paste(directory, "Males_sig_SNPs_annotations_nearest_twoplusmethods.txt", sep="")
write.table(genes_male_combined, file = outfile, sep = "\t", row.names = FALSE, quote = FALSE)
...
male_large_table <- snps_genes_male %>%
  full_join(genes_male_combined) %>%
  select(rsID, Chr, Pos, "Effect Allele", "Other Allele",
         "Beta (Males)", "SE (Males)", "P (Males)", "Freq Effect Allele (Males)",
         "Beta (Females)", "SE (Females)", "P (Females)", "Freq Effect Allele (Females)",
...
# Save
outfile <- paste(directory, "Males_sig_SNPs_GWAS_results_annotations_nearest_twoplusmethods_pos_eqtl_ci.txt", sep="")
write.table(male_large_table, file = outfile, sep = "\t", row.names = FALSE, quote = FALSE)
