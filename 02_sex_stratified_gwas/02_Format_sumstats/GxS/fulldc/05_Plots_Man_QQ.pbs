...
name=$(echo ${files[${PBS_ARRAY_INDEX}]} | sed 's|/path/01_Format_sumstats/GxS/fulldc/||' | sed 's|_MDD_GxS_fulldc_sumstats_formatted_formetaanalysis_transformed.txt||')
...
Rscript --vanilla ${reference}GEI_Plots_Man_QQ_all_SNPs_onlyGE.R ${name}_temp.txt ${name}_MDD_GxS_fulldc_sumstats_formatted_formetaanalysis_transformed ${p_threshold}
