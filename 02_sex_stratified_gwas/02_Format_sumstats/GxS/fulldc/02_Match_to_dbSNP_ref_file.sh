...
directory=/media/desk15/iy2120/Project2026/Myproject_AD/02_sex_stratified_gwas/010_Format_sumstats/GxS/fulldc/ # This is where the plots will be output
...
files=/media/desk15/iy2120/Project2026/Myproject_AD/02_sex_stratified_gwas/010_Format_sumstats/GxS/fulldc/*_ad_GxS_fulldc_sumstats.txt

#name=$(echo ${files} | sed 's/\/media\/desk15\/iy2120\/Project2026\/Myproject_AD\/02_sex_stratified_gwas\/010_Format_sumstats\/GxS\/fulldc\///g' | sed 's/_build37_ad_GxS_fulldc_sumstats.txt//g')
...
for FILE in ${files}
do
name=$(echo ${FILE} | sed 's/\/media\/desk15\/iy2120\/Project2026\/Myproject_AD\/02_sex_stratified_gwas\/010_Format_sumstats\/GxS\/fulldc\///g' | sed 's/_build37_ad_GxS_fulldc_sumstats.txt//g')
...
if [[ "${FILE}" == *"build37"* ]]; then
    ...
    awk 'NR==FNR {
         ...
         }' ${dbSNP} ${FILE} > ${name}_ad_GxS_fulldc_sumstats_formatted.txt
...
awk '($16 != "missing" && !($16 ~ /^no_match/) && $17 != "no_rsid") {print}' ${name}_ad_GxS_fulldc_sumstats_formatted.txt > ${name}_ad_GxS_fulldc_sumstats_formatted_formetaanalysis.txt
