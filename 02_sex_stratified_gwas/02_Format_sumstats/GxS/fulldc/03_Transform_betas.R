...
ADNI = paste0(directory, "adni_ad_GxS_fulldc_sumstats_formatted_formetaanalysis.txt", sep = "")

ROSMAP = paste0(directory, "rosmap_ad_GxS_fulldc_sumstats_formatted_formetaanalysis.txt", sep = "")

MSBB = paste0(directory, "msbb_ad_GxS_fulldc_sumstats_formatted_formetaanalysis.txt", sep = "")
...
ADNI <- read.table(ADNI, header = T, stringsAsFactors = F)

ROSMAP <- read.table(ROSMAP, header = T, stringsAsFactors = F)

MSBB <- read.table(MSBB, header = T, stringsAsFactors = F)
...
process_dataframe <- function(df, df_name, cases, controls, directory){
  ...
  # Save the plot
  plot_file <- paste0(directory, "/", df_name, "_check_P_vs_Ptransformed.pdf")
  ggsave(plot, width = 12, height = 10, unit = "cm", file = plot_file)
  
  # Save the transformed dataframe to a text file
  output_file <- paste0(directory, "/", df_name, "_ad_GxS_fulldc_sumstats_formatted_formetaanalysis_transformed.txt")
...
ADNI_transformed <- process_dataframe(ADNI, "adni", cases = 173, controls = 401, directory)
ROSMAP_transformed <- process_dataframe(ROSMAP, "rosmap", cases = 384, controls = 718, directory)
MSBB_transformed <- process_dataframe(MSBB, "msbb", cases = 178, controls = 167, directory)
