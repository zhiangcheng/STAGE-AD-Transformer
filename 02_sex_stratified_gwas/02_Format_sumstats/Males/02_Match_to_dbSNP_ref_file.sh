...
files=/media/desk15/iy2120/Project2026/Myproject_AD/02_sex_stratified_gwas/010_Format_sumstats/Males/*_ad_male_sumstats.txt

#name=$(echo ${files} | sed 's/\/media\/desk15\/iy2120\/Project2026\/Myproject_AD\/02_sex_stratified_gwas\/010_Format_sumstats\/Males\///g' | sed 's/_build37_ad_male_sumstats.txt//g')
...
for FILE in ${files}
do
name=$(echo ${FILE} | sed 's/\/media\/desk15\/iy2120\/Project2026\/Myproject_AD\/02_sex_stratified_gwas\/010_Format_sumstats\/Males\///g' | sed 's/_build37_ad_male_sumstats.txt//g')
...
if [[ "${FILE}" == *"build37"* ]]; then
    ...
    awk 'NR==FNR {
         ...
         }' ${dbSNP} ${FILE} > ${name}_ad_male_sumstats_formatted.txt
