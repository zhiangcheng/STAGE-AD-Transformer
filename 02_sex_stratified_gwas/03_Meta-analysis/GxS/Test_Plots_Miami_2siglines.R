...
### Load Data ###
GWAS_df <- read.table(infile, header = TRUE, stringsAsFactors = F, colClasses = c(P = "character"))

GWAS_df2 <- read.table(infile2, header = TRUE, stringsAsFactors = F, colClasses = c(P = "character"))
...
#combined_df$P <- pmax(as.numeric(combined_df$P), 1e-20)
