...
directory=/path/03_Metal/GxS/fulldc/ # This is where the plots will be output
...
files=(/path/03_Metal/GxS/fulldc/Metaanalysis_MDD_GxS_fulldc_*_QCed_rsID.txt)

name=$(echo ${files[${PBS_ARRAY_INDEX}]} | sed 's|/path/03_Metal/GxS/fulldc/||' | sed 's|_QCed_rsID.txt||')
