...
original = "/media/desk15/iy2120/TWAS/Breast-cancer-Example/pipeline/05_focus/input/patched/sqtl_dbs/before_patched/mashr_Adipose_Visceral_Omentum.db"
create_empty_template(original, "../input/template_db/empty_template_sqtl.db")


og_dbs = glob.glob("/media/desk15/iy2120/TWAS/Breast-cancer-Example/pipeline/05_focus/input/patched/sqtl_dbs/before_patched/*.db")
num_og = len(og_dbs)
...
os.makedirs("../input/patched/sqtl_dbs", exist_ok=True)

num_done = 0
for index, single_db in enumerate(og_dbs):
    target = f"../input/patched/sqtl_dbs/{os.path.basename(single_db)}"
    os.system(f"cp ../input/template_db/empty_template_sqtl.db {target}")
