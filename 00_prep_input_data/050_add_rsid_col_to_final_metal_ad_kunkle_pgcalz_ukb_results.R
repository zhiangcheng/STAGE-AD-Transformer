YOUR_METAL_FILE="~/TWAS/Breast-cancer-Example/data/subtype_gwas/output/metal/final_metal_bcac-white_ukb1.txt"
OUT_METAL_PATH="~/TWAS/Breast-cancer-Example/data/subtype_gwas/output/metal/rsid_final_metal_bcac-white_ukb1.txt"
...
main <- function(metal_file_path, out_file_path,
  ...
  ukb_path <- "~/TWAS/Breast-cancer-Example/data/subtype_gwas/input/Anno_res_anyBC_full_chrom.breast.logistic.gz"
  
  ukb_rsid_key <- fread(ukb_path, select=c("rsID", "var_name")) %>%
    filter(str_detect(rsID, "rs"))
  print(glue("Read file: {ukb_path}"))
  
  bcac_path <- "~/TWAS/Breast-cancer-Example/data/subtype_gwas/input/oncoarray_bcac_public_release_oct17.txt.gz"
